...
const processMessage = async (req, res) => {
    try {
        ...
        let chat = await Chat_1.Chat.findOne({ userId: user._id });
        if (!chat) {
            chat = new Chat_1.Chat({ userId: user._id, messages: [] });
        }
        if (!user.isActivated) {
            if (user.code !== message) {
                const data = {
                    role: "assistant",
                    content: "Hello! I notice you haven't activated your account yet. To get personalized nutrition advice, please generate an activation code from your profile page on our web app. Once you have the code, send it to me and I'll help you get started with your nutrition journey! In the meantime, I can't provide specific advice, but I'm looking forward to helping you achieve your health goals.",
                };
                chat.messages.push({
                    content: message,
                    isFromUser: true,
